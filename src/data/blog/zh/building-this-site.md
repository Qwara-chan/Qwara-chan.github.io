...
选择 Pagefind 而非 Fuse.js 的原因：

- **构建时索引**：在 `astro build` 后运行 `pagefind --site dist`，自动扫描 HTML 生成索引
- **按需加载**：索引分块存储，搜索时只加载相关分片
- **原生多语言支持**：自动识别中英文内容
- **零配置 UI**：提供现成的 Web Component
